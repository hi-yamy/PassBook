...
public class PassTemplateRowMapper implements RowMapper<PassTemplate> {
    ...
    private static  byte[]  FAMLIY_C =Constants.PassTemplateTable.FAMLIY_C.getBytes();
    ...
    public PassTemplate mapRow(Result result, int i) throws Exception {
        ...
        passTemplate.setLimit(Bytes.toLong(result.getValue(FAMLIY_C,LIMIT)));
        passTemplate.setStart(DateUtils.parseDate(Bytes.toString(result.getValue(FAMLIY_C,START)),patterns));
        passTemplate.setEnd(DateUtils.parseDate(Bytes.toString(result.getValue(FAMLIY_C,END)),patterns));
        return passTemplate;
    }
}
